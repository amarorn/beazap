...
export async function callWebHook(
  ...
  if (webhook) {
    ...
    try {
      ...
      // data por ultimo sobrescreveria event/session; manter evento e sessao estaveis no body
      data = Object.assign({}, data, { event: event, session: client.session });
      ...
      api
        ...
        .catch((e: any) => {
          const cause =
            ...
            e?.message;
          req.logger.warn(
            `Error calling Webhook url=${webhook} cause=${cause || String(e)}`
          );
        });
    } catch (e) {
      req.logger.error(e);
